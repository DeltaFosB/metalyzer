...
static PassMetrics run_c_subset_pass(const std::string &input_buffer) {
...
void worker_c_subset_execution(const TargetPayload target, int target_core_id,
  ...
  if (!file_stream.is_open()) {
    ...
    std::cerr << "\n[Benchmark Error] C Subset input file missing at "
              << target.path << "\n";
    return;
  }
  ...
  for (int i = 0; i < STATISTICAL_ITERATIONS; ++i) {
    ...
    history_pass1.push_back(run_c_subset_pass(flat_input_payload));
    history_pass2.push_back(run_c_subset_pass(flat_input_payload));
    history_pass3.push_back(run_c_subset_pass(flat_input_payload));
  }
