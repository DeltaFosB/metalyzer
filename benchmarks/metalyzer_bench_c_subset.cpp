...
static PassMetrics run_c_subset_pass(std::ifstream &file_stream) {
...
void worker_c_subset_execution(const TargetPayload target, int target_core_id,
  ...
  for (int i = 0; i < STATISTICAL_ITERATIONS; ++i) {
    ...
    history_pass1.push_back(run_c_subset_pass(file_stream));
    history_pass2.push_back(run_c_subset_pass(file_stream));
    history_pass3.push_back(run_c_subset_pass(file_stream));
  }
