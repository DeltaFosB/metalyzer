...
void worker_core_execution(const TargetPayload target, int target_core_id,
  ...
  if (!file_stream.is_open()) {
    ...
    std::cerr << "\n[Flex Benchmark Error] Input file missing at "
              << target.path << "\n";
    return;
  }
  ...
  for (int i = 0; i < STATISTICAL_ITERATIONS; ++i) {
    ...
    history_pass1.push_back(run_flex_pass(flat_input_payload));
    history_pass2.push_back(run_flex_pass(flat_input_payload));
    history_pass3.push_back(run_flex_pass(flat_input_payload));
  }
...
int main() {
#ifndef BENCH_FILE_PREFIX
#define BENCH_FILE_PREFIX "calc"
#endif
#ifndef BENCH_SUITE_LABEL
#define BENCH_SUITE_LABEL "CALCULATOR"
#endif

  std::string prefix = BENCH_FILE_PREFIX;
  std::vector<TargetPayload> payloads = {
      {"DENSE_CODE", "../benchmarks/data/" + prefix + "_dense.txt"},
      {"SPARSE_SPACES", "../benchmarks/data/" + prefix + "_sparse.txt"},
      {"ERROR_CHURN", "../benchmarks/data/" + prefix + "_error.txt"}};

  global_active_threads = static_cast<int>(payloads.size());

  std::cout << "==============================================================="
               "==================\n";
  std::cout << "INDUSTRY-STANDARD FLEX (" << BENCH_SUITE_LABEL
            << ") MULTI-CORE PARALLEL LABORATORY\n";
  std::cout << "==============================================================="
               "==================\n";
  ...
  workers.reserve(global_active_threads);
