...
#undef yyFlexLexer
#define yyFlexLexer CSubSpecFlexLexer
#include <FlexLexer.h>

static PassMetrics run_flex_c_subset_pass(const std::string &input_buffer) {
  ZeroCopyReadOnlyBuf raw_buffer(input_buffer.data(), input_buffer.size());
  std::istream stream(&raw_buffer);

  CSubSpecFlexLexer lexer(&stream);
...
void worker_flex_c_subset_execution(const TargetPayload target,
  ...
  if (!file_stream.is_open()) {
    ...
    std::cerr << "\n[Flex Benchmark Error] C Subset input file missing at "
              << target.path << "\n";
    return;
  }
  ...
  for (int i = 0; i < STATISTICAL_ITERATIONS; ++i) {
    if (i % 5 == 0 || i == STATISTICAL_ITERATIONS - 1) {
      ...
      std::cout << "\033[" << lines_to_move << "A\r\033[K"
                << "    -> [Flex_C_Sub_" << target.name << " on Core "
    ...
    history_pass1.push_back(run_flex_c_subset_pass(flat_input_payload));
    history_pass2.push_back(run_flex_c_subset_pass(flat_input_payload));
    history_pass3.push_back(run_flex_c_subset_pass(flat_input_payload));
  }
  ...
  ThreadResult record{
      "C_Subset_" + target.name, target_core_id,     agg1, agg2, agg3,
