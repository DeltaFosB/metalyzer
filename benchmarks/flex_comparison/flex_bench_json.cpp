...
#undef yyFlexLexer
#define yyFlexLexer JsonSpecFlexLexer
#include <FlexLexer.h>

static PassMetrics run_flex_json_pass(const std::string &input_buffer) {
  ZeroCopyReadOnlyBuf raw_buffer(input_buffer.data(), input_buffer.size());
  std::istream stream(&raw_buffer);

  JsonSpecFlexLexer lexer(&stream);
...
void worker_flex_json_execution(const TargetPayload target, int target_core_id,
  ...
  if (!file_stream.is_open()) {
    ...
    std::cerr << "\n[Flex Benchmark Error] JSON input file missing at "
              << target.path << "\n";
    return;
  }
  ...
  for (int i = 0; i < STATISTICAL_ITERATIONS; ++i) {
    if (i % 5 == 0 || i == STATISTICAL_ITERATIONS - 1) {
      ...
      std::cout << "\033[" << lines_to_move << "A\r\033[K"
                << "    -> [Flex_JSON_" << target.name << " on Core "
    ...
    history_pass1.push_back(run_flex_json_pass(flat_input_payload));
    history_pass2.push_back(run_flex_json_pass(flat_input_payload));
    history_pass3.push_back(run_flex_json_pass(flat_input_payload));
  }
