...
static PassMetrics run_json_pass(const std::string &input_buffer) {
...
void worker_json_execution(const TargetPayload target, int target_core_id,
  ...
  for (int i = 0; i < STATISTICAL_ITERATIONS; ++i) {
    ...
    history_pass1.push_back(run_json_pass(flat_input_payload));
    history_pass2.push_back(run_json_pass(flat_input_payload));
    history_pass3.push_back(run_json_pass(flat_input_payload));
  }
