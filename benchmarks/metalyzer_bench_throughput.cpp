...
// Forward link to isolated grammar engines compiled in separate files
void worker_calculator_execution(const TargetPayload target, int target_core_id,
                                 int display_row_offset);
void worker_json_execution(const TargetPayload target, int target_core_id,
                           int display_row_offset);
void worker_c_subset_execution(const TargetPayload target, int target_core_id,
                               int display_row_offset);
...
int main() {
  // Structural layout arrays defining the 9-point benchmarking payload matrix
  ...
  // -------------------------------------------------------------------------
  // BATCH 1: Calculator Grammar Sweep
  // -------------------------------------------------------------------------
  {
    ...
    for (int i = 0; i < 3; ++i) {
      batch_workers.push_back(std::thread(
          worker_calculator_execution, calc_payloads[i], hardware_cores[i], i));
    }
  ...
  // -------------------------------------------------------------------------
  // BATCH 2: JSON Core Grammar Sweep
  // -------------------------------------------------------------------------
  {
    global_active_threads = 3;
    std::vector<std::thread> batch_workers;
    ...
    for (int i = 0; i < 3; ++i) {
      batch_workers.push_back(std::thread(
          worker_json_execution, json_payloads[i], hardware_cores[i], i));
    }
    for (auto &worker : batch_workers) {
      if (worker.joinable())
        worker.join();
  ...
  // -------------------------------------------------------------------------
  // BATCH 3: C Subset Grammar Sweep
  // -------------------------------------------------------------------------
  {
    global_active_threads = 3;
    std::vector<std::thread> batch_workers;
    ...
    for (int i = 0; i < 3; ++i) {
      batch_workers.push_back(std::thread(
          worker_c_subset_execution, c_sub_payloads[i], hardware_cores[i], i));
    }
    for (auto &worker : batch_workers) {
      if (worker.joinable())
        worker.join();
  ...
  // Serialize aggregated, interference-free run history fields
  write_json_output("metalyzer_baseline_metrics.json");

  std::cout << "\n[+] Data acquisition completed. Pristine hardware metrics "
               "serialized to 'metalyzer_baseline_metrics.json'\n";
  std::cout << "==============================================================="
               "==================\n";
  return 0;
}
