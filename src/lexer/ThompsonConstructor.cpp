...
NFA ThompsonConstructor::createUnion(NFA left, NFA right){
  ...
  union_nfa.allStates.insert(union_nfa.allStates.end(), left.allStates.begin(), left.allStates.end());
  union_nfa.allStates.insert(union_nfa.allStates.end(), right.allStates.begin(), right.allStates.end());
...
NFA ThompsonConstructor::createConcat(NFA left, NFA right){
  ...
  concat_nfa.allStates.insert(concat_nfa.allStates.end(), left.allStates.begin(), left.allStates.end());
  concat_nfa.allStates.insert(concat_nfa.allStates.end(), right.allStates.begin(), right.allStates.end());
...
NFA ThompsonConstructor::createStar(NFA base){
  ...
  star_nfa.allStates.insert(star_nfa.allStates.end(), base.allStates.begin(), base.allStates.end());
...
NFA ThompsonConstructor::build(const std::string& regexPattern){
  ...
  std::stack<NFA> eval;
  for(char ch : regex){
    if(isalnum(ch)) eval.push(createBasic(ch));
    else if(ch == '*') {
      if (eval.empty()) continue;
      NFA base = eval.top();
      eval.pop();
      ...
      if (ch == '|') eval.push(createUnion(left, right));
      else eval.push(createConcat(left, right));
    }
  }

  return eval.empty() ? NFA{} : eval.top();
};
